...
struct ReviewTargetTests {
    @Test func reviewTargetDecodesCurrentContractValues() throws {
        let uncommitted = try JSONDecoder().decode(ReviewTarget.self, from: Data(#"{"type":"uncommitted"}"#.utf8))
        let branch = try JSONDecoder().decode(ReviewTarget.self, from: Data(#"{"type":"branch","branch":"main"}"#.utf8))
        ...
        #expect(uncommitted == .uncommitted)
    ...
    @Test func reviewTargetRejectsLegacyContractValues() {
        #expect(throws: DecodingError.self) {
            _ = try JSONDecoder().decode(ReviewTarget.self, from: Data(#"{"type":"uncommittedChanges"}"#.utf8))
        }
        #expect(throws: DecodingError.self) {
            _ = try JSONDecoder().decode(ReviewTarget.self, from: Data(#"{"type":"baseBranch","branch":"main"}"#.utf8))
        }
    }
}
